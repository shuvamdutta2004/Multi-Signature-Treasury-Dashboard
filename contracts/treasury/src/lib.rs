...
contractmeta!(
    key = "Description",
    val = "Multi-Signature Treasury: collective spend management for a shared on-chain treasury"
);
...
fn emit_treasury_initialized(env: &Env, threshold: u32, signer_count: u32) {
    let topics = (Symbol::new(env, "treasury_initialized"),);
    env.events().publish(topics, (threshold, signer_count));
}

fn emit_deposit_received(env: &Env, from: &Address, amount: i128) {
    let topics = (Symbol::new(env, "deposit_received"), from.clone());
...
impl TreasuryContract {
    /// Initialize the treasury with a set of signers and a threshold.
    /// Can only be called once. threshold must be <= signers.len().
    pub fn initialize(env: Env, signers: Vec<Address>, threshold: u32) {
        ...
        emit_treasury_initialized(&env, threshold, signer_count);
        log!(&env, "Treasury initialized: threshold={}, signers={}", threshold, signer_count);
    }
    ...
    pub fn deposit(env: Env, from: Address, token_address: Address, amount: i128) {
        ...
        emit_deposit_received(&env, &from, amount);
        log!(&env, "Deposit received: from={:?}, amount={}", from, amount);
    }

    /// Create a new spend proposal. Only signers can propose.
